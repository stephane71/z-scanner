...
describe('MarketPicker', () => {
  ...
  const mockMarkets: (Market & { id: number })[] = [
    { id: 1, name: 'Marché Aligre', userId: mockUserId, createdAt: '2026-01-01T00:00:00Z' },
    { id: 2, name: 'Marché Bastille', userId: mockUserId, createdAt: '2026-01-02T00:00:00Z' },
  ];
  ...
  it('lists markets sorted alphabetically by name', () => {
    ...
    const marketButtons = screen.getAllByRole('button').filter((btn) =>
      btn.textContent?.includes('Marché')
    );
  ...
  it('shows checkmark indicator on selected market', () => {
    ...
    const bastilleButton = screen.getByText('Marché Bastille').closest('button');
  ...
  it('renders "+ Ajouter un marché" button that reveals the quick-add form', async () => {
    ...
    expect(screen.queryByPlaceholderText('Nom du marché')).not.toBeInTheDocument();
  ...
  it('shows loading state during market creation', async () => {
    ...
    // Make addMarket slow
    mockAddMarket.mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve(3), 100)));
