...
vi.mock("@/lib/utils/date-ranges", async (importOriginal) => {
  const original =
    await importOriginal<typeof import("@/lib/utils/date-ranges")>();
  return {
    ...original,
    getThisMonth: vi.fn(() => ({
      start: "2026-01-01",
...
const mockUseExportPreview = useExportPreview as unknown as ReturnType<
  typeof vi.fn
>;
const mockUseGenerateExport = useGenerateExport as unknown as ReturnType<
  typeof vi.fn
>;
const mockUseDownloadCsv = useDownloadCsv as unknown as ReturnType<
  typeof vi.fn
>;
...
describe("ExportPageClient", () => {
  ...
  describe("CSV generation", () => {
    ...
    it("shows error message when CSV generation fails", async () => {
      ...
      expect(
        screen.getByText("Erreur: Failed to generate CSV"),
      ).toBeInTheDocument();
    });
  });
